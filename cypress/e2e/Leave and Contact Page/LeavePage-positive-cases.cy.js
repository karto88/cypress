...
// end date არჩევა
  ...
  it('live list - Clicks next month button 4 times left', () => {
    ...
    cy.get('.oxd-calendar-selector-month-selected > .oxd-text', { timeout: 10000 })
  .invoke('text').then(text => {
    expect(text).to.eq('December')
  });
  })
...
// end date არჩევა
   ...
   cy.get('.oxd-button--secondary').click() // search
    ...
    const notfoundtext = 'InfoNo Records Found'
    cy.get('.oxd-toast p').then((checknotfound) => {
      const exptext = checknotfound.text();
      expect(exptext).to.equal(notfoundtext)
    })
